...
task :post do
  ...
  open(filename, 'w') do |post|
    ...
    post.puts ""
    post.puts "<!--  The way to include an image is like so, eg.: !(img-right)/assets/themes/twitter/images/posts/2012-04-09-en-2011/3.jpg!  -->"
    post.puts ""
    post.puts "<!--  The way to include a video is like so, eg.: \<div align='center'\>\<iframe src='http://player.vimeo.com/video/26805867?portrait=0' width='400' height='300' frameborder='0' \>\</iframe\>\</div\>   -->"
    post.puts "<!--  Just substitute in the video number as seen in the vimeo link -->"
    post.puts ""
    post.puts "<!-- Here is the link for you to refer to textile syntax: http://en.wikipedia.org/wiki/Textile_(markup_language) -->"
    post.puts ""
    post.puts ""
    post.puts ""
